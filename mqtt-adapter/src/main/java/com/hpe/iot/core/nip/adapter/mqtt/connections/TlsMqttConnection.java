 ...
 * 
 */
public class TlsMqttConnection extends PlainMqttConnection {
	 ...
	 * @param brokerUrl
	 */
	public TlsMqttConnection(String brokerUrl) {
		super(brokerUrl);

		try {
			// クライアント認証
			char[] keyPass = "ippush".toCharArray();
			KeyStore keyStore = KeyStore.getInstance("JKS");
			keyStore.load(this.getClass().getResourceAsStream("/tmcKeystore.jks"), keyPass);
			KeyManagerFactory kmf = KeyManagerFactory.getInstance("SunX509");
			kmf.init(keyStore, keyPass);
			
			// サーバ認証
			keyPass = "changeit".toCharArray();
			KeyStore trustStore = KeyStore.getInstance("JKS");
			trustStore.load(this.getClass().getResourceAsStream("/cacerts"), keyPass);
